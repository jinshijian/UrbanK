...
# scale_fill_discrete(name = "Data", labels=c('Training', 'Verification')) +
  xlab(expression( K[fs]~","~log~(cm~h^{-1}) )) +
  ylab('Density')
...
read.csv(here::here("extdata/AllCities_Victoria_RDS_rock.csv")) %>% 
  ...
  select(BulkDensity_gcm3, Unsaturated_K2cm_cmhr, Percent_Sand,
         Percent_Silt, Percent_Clay, hwsd_bd, Texture) -> 
  bd_test_data 
...
ggsave("Figure A4.jpg", width = 6, height = 6, dpi = 300, units = "in")
```
